...
class ScrapingService {
  ...
  Future<Show> getAnoboyShowDetails(Show show) async {
    ...
    try {
      ...
      if (parentShowUrl != null &&
          parentShowUrl != show.originalUrl &&
  ...
  String? _findAnoboyParentShowUrl(Document document) {
    ...
    if (parentShowUrl == null) {
      try {
        final allEpLink = document
            .querySelectorAll('a')
            .firstWhere(
              (a) =>
                  a.text.toLowerCase().contains('semua episode') ||
    ...
    if (rows.isNotEmpty) {
      for (var row in rows) {
        ...
        if (cols.length >= 2) {
          ...
          } else if ((key.contains('skor') || key.contains('score')) &&
      ...
      final contentText =
          document.querySelector('.entry-content, .post-body')?.text ?? '';
  ...
  Future<Episode> getAnoboyEpisodeDetails(Episode episode) async {
    ...
    try {
      ...
      for (var mirror in mirrorElements) {
        final link =
            mirror.attributes['data-video'] ?? mirror.attributes['href'];
      ...
      for (var dl in dlElements) {
        ...
        if (link != null && link.isNotEmpty && link != 'none') {
          ...
          if (parent != null) {
            final providerText =
                parent.querySelector('.udj')?.text.trim() ?? '';
    ...
    for (var link in epLinks) {
      ...
      bool containsEpOrKeyword = title.toLowerCase().contains('episode') ||
          ...
          (cleanBaseShowTitle != null &&
              cleanBaseShowTitle.isNotEmpty &&
              title.toLowerCase().contains(cleanBaseShowTitle.split(' ')[0]));

      if (containsEpOrKeyword) {
        if (cleanBaseShowTitle != null &&
            cleanBaseShowTitle.isNotEmpty &&
            showTitle != null &&
          ...
          final cleanTitleForMatch = normalizedTitle
              ...
              .replaceAll(RegExp(r'[^a-z0-9\s]'), ' ')
              .replaceAll(RegExp(r'\s+'), ' ');
          final hasKeyword = cleanBaseShowTitle
              .split(' ')
              .where((w) => w.length > 2)
              .any((w) => cleanTitleForMatch.contains(w));
    ...
    try {
      ...
      if (dlWrappers.isNotEmpty) {
        for (var wrapper in dlWrappers) {
          final resLabel =
              wrapper.querySelector('.sorattl')?.text.trim() ?? 'Download';
          ...
          for (var element in content.children) {
            ...
            if (days.any((d) => text.contains(d) && text.length < 20)) {
              ...
              schedule[currentDay] = [];
                ...
                schedule.containsKey(currentDay)) {
              final links = element.localName == 'a'
                  ? [element]
