...
class ScrapingService {
  ...
  Future<Show> getAnoboyShowDetails(Show show) async {
    ...
    try {
      ...
      if (parentShowUrl != null && parentShowUrl != show.originalUrl && !parentShowUrl.contains('/000/')) {
         try {
  ...
  String? _findAnoboyParentShowUrl(Document document) {
    ...
    if (parentShowUrl == null || parentShowUrl.contains('/000/')) {
      parentShowUrl = null; // Ignore invalid breadcrumb
      final bcs = document.querySelectorAll('.breadcrumb a, .breadcrumbs a');
      if (bcs.isNotEmpty) {
  ...
  List<Episode> _parseAnoboyEpisodesFromDoc(dynamic document, int showId, {String? showTitle}) {
    ...
    if (listContainers.isNotEmpty) {
      for (var container in listContainers) {
    ...
    if (showTitle != null && showTitle.isNotEmpty) {
      ...
      cleanBaseShowTitle = normalizedShowTitle
          .replaceAll(RegExp(r'(season|part|cour)\s*\d+(\s*(\+|&|dan|/)\s*\d+)*', caseSensitive: false), '')
    ...
    for (var link in epLinks) {
      ...
      if (containsEpOrKeyword) {
        if (cleanBaseShowTitle != null && cleanBaseShowTitle.isNotEmpty && showTitle != null && showTitle.isNotEmpty) {
           final normalizedTitle = title.toLowerCase();
           final isGenericFormat = RegExp(r'^(?:episode|ep|part)\s+\d+', caseSensitive: false).hasMatch(normalizedTitle.trim());
           final cleanTitleForMatch = normalizedTitle.replaceAll('“', '').replaceAll('”', '').replaceAll(RegExp(r'[^a-z0-9\s]'), ' ').replaceAll(RegExp(r'\s+'), ' ');
           final hasKeyword = cleanBaseShowTitle.split(' ').where((w) => w.length > 2).any((w) => cleanTitleForMatch.contains(w));

           if (!isGenericFormat && !hasKeyword) {
              continue;
           }
        }

        int epNum = 0;
        final epMatch = RegExp(r'(?:Episode|Ep|Part|Cour)\s+(\d+)', caseSensitive: false).firstMatch(title) ?? RegExp(r'(?:Episode|Ep|Part|Cour).*\s+(\d+)', caseSensitive: false).firstMatch(title);
        final isDownload = title.toLowerCase().contains('download') && (cleanBaseShowTitle != null && title.toLowerCase().contains(cleanBaseShowTitle.split(' ')[0]) || cleanBaseShowTitle == null);
